                                       ...
                                       char* * outbuf, size_t *outbytesleft)
{
  ...
  while (*inbytesleft >= sizeof(wchar_t)) {
    ...
    while (inleft >= sizeof(wchar_t)) {
	  ...
	  size_t count = -1;
      errno_t err = wcrtomb_s(&count, buf+bufcount, sizeof(buf) - bufcount,*inptr,&state);
      if (err != 0 || count == (size_t)(-1)) {
        /* Invalid input. */
                                     ...
                                     char* * outbuf, size_t *outbytesleft)
{
  ...
  if (outbuf == NULL || *outbuf == NULL) {
    ...
    if (!mbsinit(&wcd->state)) {
      ...
      char buf[BUF_SIZE];
	  size_t bufcount = -1;
      errno_t err = wcrtomb_s(&bufcount, buf, sizeof(buf),(wchar_t)0,&state);
      if (err != 0 || bufcount == (size_t)(-1) || bufcount == 0 || buf[bufcount-1] != '\0')
        abort();
      else {
        const char* bufptr = buf;
